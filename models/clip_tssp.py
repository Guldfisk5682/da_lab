...
class CLIPVisualWithHidden(nn.Module):
    """CLIP ViT with a clean hidden-state path and optional persistent VCTX."""

    def __init__(self, visual, enable_vpt=False, n_vctx=8, vctx_init_std=0.02):
        super().__init__()
        ...
        if self.enable_vpt and self.n_vctx <= 0:
            raise ValueError(
                f"n_vctx must be positive when VPT is enabled, got {n_vctx}"
            )

        if self.enable_vpt:
            self.vctx = nn.Parameter(torch.empty(self.n_vctx, self.width))
            nn.init.normal_(self.vctx, std=float(vctx_init_std))
        else:
    ...
    def forward(self, image):
        ...
        x = x.permute(1, 0, 2)
        image_features = self._project_cls(x)

        return image_features, hidden_states

    def forward_vpt(self, image):
        if not self.enable_vpt:
            raise RuntimeError("forward_vpt requires ENABLE_VPT=True")
