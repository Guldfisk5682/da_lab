...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "Usage: bash scripts/maple_continuous_mtda/run_officehome_one.sh <A|C|P|R> <SEED> [--debug]" >&2
  exit 1
fi
...
CFG="${CFG:-${REPO_ROOT}/configs/trainers/ContinuousMaPLeMTDA/vit_b16.yaml}"
EXTRA_OPTS="${EXTRA_OPTS:-}"
METHOD_TAG="${METHOD_TAG:-maple_continuous_mtda}"
DRY_RUN="${DRY_RUN:-0}"
...
echo "ContinuousMaPLeMTDA Office-Home SS-MTDA"
echo "  repo: ${REPO_ROOT}"
