...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "Usage: bash scripts/maple_gapctx_mtda/run_officehome_one.sh <A|C|P|R> <SEED> [--debug]" >&2
  exit 1
fi
...
CFG="${CFG:-${REPO_ROOT}/configs/trainers/ContinuousSharedProjGapCtxPLMaPLeMTDA/vit_b16.yaml}"
EXTRA_OPTS="${EXTRA_OPTS:-}"
METHOD_TAG="${METHOD_TAG:-maple_gapctx_mtda}"
DRY_RUN="${DRY_RUN:-0}"
...
OPTS=(
  "--root" "${DATA}"
  "--seed" "${SEED}"
  "--trainer" "ContinuousSharedProjGapCtxPLMaPLeMTDA"
...
echo "ContinuousSharedProjGapCtxPLMaPLeMTDA Office-Home SS-MTDA"
echo "  repo: ${REPO_ROOT}"
