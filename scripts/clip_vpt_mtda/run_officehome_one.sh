...
if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
  echo "Usage: bash scripts/clip_vpt_mtda/run_officehome_one.sh <A|C|P|R> <SEED> <clip_zs|clip_vpt> [--debug]" >&2
  exit 1
fi
...
CFG="${CFG:-${REPO_ROOT}/configs/trainers/CLIPVPTMTDA/vit_b16.yaml}"
BACKBONE="${BACKBONE:-}"
...
case "${METHOD}" in
  clip_zs|zeroshot|zs)
    CMD+=(--eval-only)
    METHOD_OPTS=("TRAINER.CLIP_VPT_MTDA.ENABLE_VPT" "False")
    ;;
  clip_vpt|vpt)
    METHOD_OPTS=("TRAINER.CLIP_VPT_MTDA.ENABLE_VPT" "True")
    ;;
  *)
...
if [ "${DEBUG_FLAG}" = "--debug" ]; then
  CMD+=(
    "TRAINER.CLIP_VPT_MTDA.DEBUG.PRINT_ONCE" "True"
