...
SEED="$2"
if [ "${SOURCE}" != "C" ]; then
  echo "Student top-1 mechanism control is restricted to C2APR, got ${SOURCE}" >&2
  exit 2
fi

ORDER="art real_world product"
TARGET_TAG="APR"
METHOD_TAG="maple_dualpl_agreement_hard_student_top1_seed${SEED}"
...
PL_VARIANT=agreement_hard_student_top1 \
...
echo "Student top-1 mechanism control completed: ${SOURCE}2${TARGET_TAG} seed${SEED}"
